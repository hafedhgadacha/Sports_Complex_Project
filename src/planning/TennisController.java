...
public class TennisController implements Initializable {
  ...
  @Override
    ...
    try (Connection connection = complexe_sportif.database.getConnect()) {
        String query = "SELECT * FROM planningtennis WHERE lineof = 1";
...
private void setButtonStyle(Button button, String cellData,String colrow) {
     if (cellData == null || cellData.trim().isEmpty()) {
        ...
        try (Connection connection = complexe_sportif.database.getConnect()) {
            String querySelect = "SELECT " + colrow + " FROM planningtennis WHERE lineof = 2";
      ...
      long currentTime = System.currentTimeMillis();
        if (currentTime - lastClickTime < 500) {
                                    ...
                                    String query = "UPDATE planningtennis SET  row"+row+"_col"+col+"=? ";
                            ...
                            try (ResultSet res = selectStatement.executeQuery()) {
                                if (res.next()) {
                                   ...
                                   String B= String.valueOf(b);
                                    ...
                                    String query1 = "UPDATE planningtennis SET row" + row + "_col" + col + "=? WHERE lineof=1";
                                    try (PreparedStatement updateStatement = connection.prepareStatement(query1)) {
                                        updateStatement.setString(1, text);
                                         updateStatement.executeUpdate();
                                                String query5 = "UPDATE planningtennis SET row" + row + "_col" + col + "=? WHERE lineof=2";
